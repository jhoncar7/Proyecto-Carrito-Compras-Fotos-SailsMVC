...
module.exports = {
  ...
  agregarCarroCompra: async (peticion, respuesta) => {
    ...
    if (foto) {
      peticion.addFlash('mensaje', 'La foto ya existe ene el carrito de compra')
    } else {
      await CarroCompra.create({ cliente: peticion.session.cliente.id, foto: peticion.params.fotoId })
